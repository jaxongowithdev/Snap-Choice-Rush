...
class _ItemFormViewState extends State<ItemFormView> {
  ...
  final _conditions = ['Clean', 'Dog-eared', 'Faded', 'Filed'];
  final _categories = ['Nouns', 'Verbs', 'Roots', 'Prefixes', 'Idioms', 'Cognates', 'Spelling', 'Grammar', 'Quotes', 'Sets', 'Field', 'Other'];
  ...
  Future<void> _pickPhoto(ImageSource source) async {
    try {
      ...
      final fileName = 'lemma_${DateTime.now().millisecondsSinceEpoch}${path_pkg.extension(photo.path)}';
  ...
  Future<void> _saveItem() async {
    ...
    if (_selectedContainerId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Choose a deck first')));
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(isEditing ? 'Edit card' : 'File a card')),
      body: Form(
        key: _formKey,
        child: ListView(
          ...
          children: [
            ...
            TextFormField(key: const ValueKey('item_name_field'), controller: _nameController, decoration: const InputDecoration(labelText: 'Lemma / word', hintText: 'e.g., photosynthesize, break the ice'), validator: (v) => (v == null || v.trim().isEmpty) ? 'Name this card' : null),
            ...
            const SizedBox(height: 12),
            DropdownButtonFormField<int>(key: const ValueKey('container_dropdown'), value: _selectedContainerId, decoration: const InputDecoration(labelText: 'Deck'), items: _containers?.map((c) => DropdownMenuItem(value: c.id, child: Text('${c.name} (${c.code})'))).toList(), onChanged: (v) => setState(() => _selectedContainerId = v), validator: (v) => v == null ? 'Choose a deck' : null),
            const SizedBox(height: 12),
            Row(
              children: [
            ...
            const SizedBox(height: 12),
            TextFormField(key: const ValueKey('notes_field'), controller: _notesController, decoration: const InputDecoration(labelText: 'Study note', hintText: 'Definition, root, or last quiz miss'), maxLines: 3),
            const SizedBox(height: 22),
            FilledButton(key: const ValueKey('save_item_button'), onPressed: _saveItem, child: Text(isEditing ? 'Save card' : 'File card')),
          ],
        ),
      ),
    );
