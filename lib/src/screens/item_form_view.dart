...
class _ItemFormViewState extends State<ItemFormView> {
  ...
  final _conditions = ['New', 'Recalled', 'Faded', 'Filed'];
  final _categories = ['Planet', 'Star', 'Moon', 'Galaxy', 'Nebula', 'Comet', 'Constellation', 'Myth', 'Number', 'Voyage', 'Teacher', 'Other'];
  ...
  Future<void> _pickPhoto(ImageSource source) async {
    try {
      ...
      final fileName = 'loci_${DateTime.now().millisecondsSinceEpoch}${path_pkg.extension(photo.path)}';
  ...
  Future<void> _saveItem() async {
    ...
    if (_selectedContainerId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Choose a room first')));
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(isEditing ? 'Edit locus' : 'File a locus')),
      body: Form(
        key: _formKey,
        child: ListView(
          ...
          children: [
            ...
            TextFormField(key: const ValueKey('item_name_field'), controller: _nameController, decoration: const InputDecoration(labelText: 'Locus name', hintText: 'e.g., Saturn’s rings, Orion’s belt'), validator: (v) => (v == null || v.trim().isEmpty) ? 'Name this locus' : null),
            ...
            const SizedBox(height: 12),
            DropdownButtonFormField<int>(key: const ValueKey('container_dropdown'), value: _selectedContainerId, decoration: const InputDecoration(labelText: 'Room'), items: _containers?.map((c) => DropdownMenuItem(value: c.id, child: Text('${c.name} (${c.code})'))).toList(), onChanged: (v) => setState(() => _selectedContainerId = v), validator: (v) => v == null ? 'Choose a room' : null),
            const SizedBox(height: 12),
            Row(
              children: [
                ...
                Expanded(child: DropdownButtonFormField<String>(key: const ValueKey('condition_dropdown'), value: _selectedCondition, decoration: const InputDecoration(labelText: 'Recall'), items: _conditions.map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(), onChanged: (v) { if (v != null) setState(() => _selectedCondition = v); })),
              ],
            ),
            const SizedBox(height: 12),
            ...
            const SizedBox(height: 12),
            TextFormField(key: const ValueKey('notes_field'), controller: _notesController, decoration: const InputDecoration(labelText: 'Mnemonic note', hintText: 'Image, hook, or last recall'), maxLines: 3),
            const SizedBox(height: 22),
            FilledButton(key: const ValueKey('save_item_button'), onPressed: _saveItem, child: Text(isEditing ? 'Save locus' : 'File locus')),
          ],
        ),
      ),
    );
