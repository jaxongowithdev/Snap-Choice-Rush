...
class _ItemFormViewState extends State<ItemFormView> {
  ...
  final _conditions = ['Clean', 'Marked', 'Worn', 'Filed'];
  final _categories = ['Scripts', 'Roles', 'Props', 'Costumes', 'Lighting', 'Sound', 'Sets', 'Makeup', 'Music', 'Blocking', 'Playbills', 'Other'];
  ...
  Future<void> _pickPhoto(ImageSource source) async {
    try {
      ...
      final fileName = 'booth_${DateTime.now().millisecondsSinceEpoch}${path_pkg.extension(photo.path)}';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(isEditing ? 'Edit cue' : 'File a cue')),
      body: Form(
        key: _formKey,
        child: ListView(
          ...
          children: [
            ...
            TextFormField(key: const ValueKey('item_name_field'), controller: _nameController, decoration: const InputDecoration(labelText: 'Cue name', hintText: 'e.g., Miranda sides, Lantern prop'), validator: (v) => (v == null || v.trim().isEmpty) ? 'Name this cue' : null),
            ...
            const SizedBox(height: 12),
            DropdownButtonFormField<int>(key: const ValueKey('container_dropdown'), value: _selectedContainerId, decoration: const InputDecoration(labelText: 'Crate'), items: _containers?.map((c) => DropdownMenuItem(value: c.id, child: Text('${c.name} (${c.code})'))).toList(), onChanged: (v) => setState(() => _selectedContainerId = v), validator: (v) => v == null ? 'Choose a crate' : null),
            const SizedBox(height: 12),
            Row(
              children: [
            ...
            const SizedBox(height: 12),
            TextFormField(key: const ValueKey('notes_field'), controller: _notesController, decoration: const InputDecoration(labelText: 'Stage note', hintText: 'Cue, blocking, or last rehearsal'), maxLines: 3),
            const SizedBox(height: 22),
            FilledButton(key: const ValueKey('save_item_button'), onPressed: _saveItem, child: Text(isEditing ? 'Save cue' : 'File cue')),
          ],
        ),
      ),
    );
