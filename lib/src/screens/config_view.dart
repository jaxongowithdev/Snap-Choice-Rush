  ...
  Future<void> _exportData() async {
    try {
      ...
      await Share.shareXFiles(
        [XFile.fromData(Uint8List.fromList(jsonString.codeUnits), mimeType: 'application/json', name: 'cubby_wall_${DateTime.now().millisecondsSinceEpoch}.json')],
        text: 'Cubby Wall catalog',
      );
  ...
  Future<void> _importData() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('The current cubbies will be replaced by the file you pick.'),
        actions: [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          Text('Cubby Wall', style: GoogleFonts.fredoka(fontSize: 32, fontWeight: FontWeight.w600)),
          const SizedBox(height: 6),
          Text('A private classroom catalog. Nothing leaves this phone.', style: GoogleFonts.literata(height: 1.4)),
          const NameTag(label: 'LIGHT'),
          for (final e in const [('light', 'Daylight wall'), ('dark', 'After the bell'), ('system', 'Match the phone')])
            ListTile(
          ...
          const NameTag(label: 'COLOPHON'),
          Text('Version 1.0.0  ·  Offline classroom inventory', style: GoogleFonts.literata(fontSize: 13)),
          const SizedBox(height: 6),
          Text('No account. No tracking. Local only.', style: GoogleFonts.literata(fontSize: 13)),
        ],
      ),
    );
  }
