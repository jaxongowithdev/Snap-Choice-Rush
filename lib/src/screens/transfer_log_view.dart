    ...
    try {
      ...
      for (final log in logs) {
        if (!_itemNames.containsKey(log.itemId)) {
          ...
          _itemNames[log.itemId] = item?.name ?? 'Unknown card';
        }
        if (!_deckNames.containsKey(log.fromContainerId)) {
          final from = await _storage.getContainer(log.fromContainerId);
          _deckNames[log.fromContainerId] = from?.name ?? 'Removed deck';
        }
        if (!_deckNames.containsKey(log.toContainerId)) {
          final to = await _storage.getContainer(log.toContainerId);
          _deckNames[log.toContainerId] = to?.name ?? 'Removed deck';
        }
      }
              ...
              ? Center(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text('still', style: GoogleFonts.literata(fontSize: 28, fontWeight: FontWeight.w700, color: VisualTheme.primaryColor)),
                        Text('No shifts yet', style: GoogleFonts.literata(fontSize: 22, fontWeight: FontWeight.w700)),
                        const SizedBox(height: 8),
                        const Text('When a card changes decks, the shift lands here.', textAlign: TextAlign.center),
                      ],
                    ),
                  ),
                )
              : RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return FlashTile(
                        kind: when,
                        title: _itemNames[log.itemId] ?? 'Card',
                        meta: '${_deckNames[log.fromContainerId]}  →  ${_deckNames[log.toContainerId]}${log.notes != null && log.notes!.isNotEmpty ? '  ·  ${log.notes}' : ''}',
