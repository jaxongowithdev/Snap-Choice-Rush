...
class _TransferLogViewState extends State<TransferLogView> {
  ...
  final Map<int, String> _crateNames = {};
    ...
    try {
      ...
      for (final log in logs) {
        if (!_itemNames.containsKey(log.itemId)) {
          ...
          _itemNames[log.itemId] = item?.name ?? 'Unknown cue';
        }
        if (!_crateNames.containsKey(log.fromContainerId)) {
          final from = await _storage.getContainer(log.fromContainerId);
          _crateNames[log.fromContainerId] = from?.name ?? 'Removed crate';
        }
        if (!_crateNames.containsKey(log.toContainerId)) {
          final to = await _storage.getContainer(log.toContainerId);
          _crateNames[log.toContainerId] = to?.name ?? 'Removed crate';
        }
      }
              ...
              ? Center(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text('hold', style: GoogleFonts.cinzel(fontSize: 28, color: VisualTheme.primaryColor)),
                        Text('No cues logged', style: GoogleFonts.cinzel(fontSize: 22, fontWeight: FontWeight.w600)),
                        const SizedBox(height: 8),
                        const Text('When a cue changes crates, the call lands here.', textAlign: TextAlign.center),
                      ],
                    ),
                  ),
                )
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return PlaybillBlock(
                        title: _itemNames[log.itemId] ?? 'Cue',
                        meta: '${_crateNames[log.fromContainerId]}  →  ${_crateNames[log.toContainerId]}  ·  $when${log.notes != null && log.notes!.isNotEmpty ? '  ·  ${log.notes}' : ''}',
